...
function readOpenCodeResumeState(resumeCursor: unknown): OpenCodeResumeState | undefined {
  ...
  const cursor = resumeCursor as { openCodeSessionId?: unknown };
  return typeof cursor.openCodeSessionId === "string" && cursor.openCodeSessionId.length > 0
    ? { openCodeSessionId: cursor.openCodeSessionId }
    : undefined;
}
    ...
    const startSession: OpenCodeAdapterShape["startSession"] = Effect.fn("startSession")(
      function* (input) {
        ...
        const started = yield* Effect.gen(function* () {
          ...
          const startedExit = yield* Effect.exit(
            Effect.gen(function* () {
              ...
              // If a previous run persisted an OpenCode session id, try to
              // reattach to it — OpenCode persists sessions on disk, so the
              // conversation survives server restarts. Fall back to a fresh
              // session when the persisted session no longer exists.
              const resumeState = readOpenCodeResumeState(input.resumeCursor);
              let openCodeSessionData: { readonly id: string } | undefined;
              if (resumeState) {
                const resumed = yield* Effect.result(
                  runOpenCodeSdk("session.get", () =>
                    client.session.get({ sessionID: resumeState.openCodeSessionId }),
                  ),
                );
                if (Result.isSuccess(resumed) && resumed.success.data) {
                  openCodeSessionData = resumed.success.data;
                } else {
                  yield* Effect.logWarning("opencode.session.resume-missing", {
                    threadId: input.threadId,
                    openCodeSessionId: resumeState.openCodeSessionId,
                  });
                }
              }
              if (!openCodeSessionData) {
                ...
                openCodeSessionData = openCodeSession.data;
              }
              return {
                sessionScope,
                server,
                client,
                openCodeSession: openCodeSessionData,
              };
        ...
        const session: ProviderSession = {
          ...
          createdAt,
          updatedAt: createdAt,
          // Persisted via the session binding so the thread can reattach to
          // the same OpenCode session after a server restart.
          resumeCursor: { openCodeSessionId: started.openCodeSession.id },
        };
