...
const runtimeMock = {
  ...
  reset() {
    ...
    this.state.getCalls.length = 0;
    this.state.existingSessionIds.length = 0;
  },
};
    ...
    ({
      session: {
        ...
        get: async ({ sessionID }: { sessionID: string }) => {
          runtimeMock.state.getCalls.push(sessionID);
          return runtimeMock.state.existingSessionIds.includes(sessionID)
            ? { data: { id: sessionID } }
            : { data: undefined };
        },
...
it.layer(OpenCodeAdapterTestLayer)("OpenCodeAdapterLive", (it) => {
  ...
  it.effect("exposes the OpenCode session id as a resume cursor", () =>
    Effect.gen(function* () {
      const adapter = yield* OpenCodeAdapter;
      ...
      NodeAssert.deepEqual(session.resumeCursor, {
        openCodeSessionId: "http://127.0.0.1:9999/session",
      });
      yield* adapter.stopSession(asThreadId("thread-opencode-cursor"));
    }),
  );

  it.effect("reattaches to a persisted OpenCode session via the resume cursor", () =>
    Effect.gen(function* () {
      const adapter = yield* OpenCodeAdapter;
      runtimeMock.state.existingSessionIds.push("persisted-session-id");

      const session = yield* adapter.startSession({
        provider: ProviderDriverKind.make("opencode"),
        threadId: asThreadId("thread-opencode-resume"),
        runtimeMode: "full-access",
        resumeCursor: { openCodeSessionId: "persisted-session-id" },
      });

      NodeAssert.deepEqual(runtimeMock.state.getCalls, ["persisted-session-id"]);
      // No new session should have been created.
      NodeAssert.deepEqual(runtimeMock.state.sessionCreateUrls, []);
      NodeAssert.deepEqual(session.resumeCursor, {
        openCodeSessionId: "persisted-session-id",
      });
      yield* adapter.stopSession(asThreadId("thread-opencode-resume"));
    }),
  );

  it.effect("falls back to a fresh session when the persisted OpenCode session is gone", () =>
    Effect.gen(function* () {
      const adapter = yield* OpenCodeAdapter;

      const session = yield* adapter.startSession({
        provider: ProviderDriverKind.make("opencode"),
        threadId: asThreadId("thread-opencode-resume-missing"),
        runtimeMode: "full-access",
        resumeCursor: { openCodeSessionId: "missing-session-id" },
      });

      NodeAssert.deepEqual(runtimeMock.state.getCalls, ["missing-session-id"]);
      NodeAssert.deepEqual(runtimeMock.state.sessionCreateUrls, ["http://127.0.0.1:9999"]);
      NodeAssert.deepEqual(session.resumeCursor, {
        openCodeSessionId: "http://127.0.0.1:9999/session",
      });
      yield* adapter.stopSession(asThreadId("thread-opencode-resume-missing"));
    }),
  );
